...
func UrlDispose(arr []mode.Link) ([]mode.Link, []mode.Link) {
	...
	for _, v := range arr {
		host := GetHost(v.Baseurl)
		if strings.Contains(v.Url, v.Baseurl) {
			urls = append(urls, v)
		} else {

			if host != "" && regexp.MustCompile(GetHost(v.Baseurl)).MatchString(v.Url) {
	...
	return RemoveDuplicatesLink(urls), RemoveDuplicatesLink(other)
}
...
func RemoveDuplicate(eletents [][]string) [][]string {
	seen := make(map[string]bool)
	var unique [][]string

	// 遍历原始二维数组
	for _, row := range eletents {
		if len(row) == 1 || row[1] == "" {
			continue
		}
		row1 := row[1]
